module ServiceBaseURLTestKit
  class ServiceBaseURLEndpointQueryGroup < Inferno::TestGroup
    ...
    description %(
      A developer's Service Base URL list must be publicly available. 
      This test issues a HTTP GET request against the supplied URL and expects to receive the service base url list at this location.
    )
    run_as_group
    ...
    test do
      ...
      description %(
        Verify that the developer's list of Service Base URLs can be publicly accessed at the supplied URL location.
      )
