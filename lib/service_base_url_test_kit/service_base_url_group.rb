module ServiceBaseURLTestKit
  class ServiceBaseURLGroup < Inferno::TestGroup
    ...
    Verify that the server makes its Service Base URL list publicly available in the Bundle resource format with valid Endpoint and Organization entries.
    ...
    returned service base URL list is in the Bundle resource format containing its service base URLs and related organizational details
    ...
    can validate a Service Base URL Bundle not served at a public endpoint 
    by running Test 1.2: Service Base URL List Validation individually.
    )
    ...
    group do
      ...
      with its service base URLs and organizational details contained in valid Endpoint and Organization 
