...
module ServiceBaseURLTestKit
  class ServiceBaseURLTestSuite < Inferno::TestSuite
    ...
    description %(
      This test kit provides a draft set of tests to validate conformance to [HTI-1](https://www.healthit.gov/topic/laws-regulation-and-policy/health-data-technology-and-interoperability-certification-program) [rule](https://www.ecfr.gov/current/title-45/subtitle-A/subchapter-D/part-170/subpart-D/section-170.404#p-170.404(b)(2)) from the API Condition and Maintenance of Certification to include the requirement for Certified API Developers with patient-facing apps to publish their service base URLs in [a specified format](https://www.federalregister.gov/d/2023-07229/p-2342).
      
      A Certified API developer must publish, at no charge, the service base URLs and related organizational details that can be used by patients to access their electronic health information. This test kit will test that a server's service base URL list conforms to the following:
        - Service base URL list is publicly accessible
        - Service based URLs are published in the Endpoint resource format      according to the standard adopted in § 170.215\(a\) - FHIR 4.0.1 release 
        - Organization details for each service base URL are published in the Organization resource format according to the standard adopted in § 170.215\(a\) - FHIR 4.0.1 release
      ...
      This test kit is provided as a preview for feedback and is not currently intended for certification. A version of these tests will be migrated into a future version of the [ONC (g)(10) Standardized API Test Kit](https://github.com/onc-healthit/onc-certification-g10-test-kit).

      While these tests do not specifically verify conformance to [Patient-Access Brands](https://build.fhir.org/ig/HL7/smart-app-launch/brands.html) within the draft SMART App Launch v2.2.0 standard, systems that implement that standard should pass these tests. Please report an issue if there are any problems.
      
      The tests within this test kit are available for developers that would like to evaluate their service list against the specified format. This is a draft set of tests and may contain errors or issues. Please provide feedback on these tests by reporting an issue in [GitHub](https://github.com/inferno-framework/service-base-url-test-kit/issues), or by reaching out to the team on the [Inferno FHIR Zulip channel](https://chat.fhir.org/#narrow/stream/179309-inferno).
    )
    version VERSION
